...
app.post("/api/search", async (req, res) => {
  ...
  try {
    ...
    const imgBuffer = await promisify(image.getBuffer).call(
      image,
      Jimp.MIME_JPEG,
    );
    ...
    const docs = await celebImagesCollection
      .aggregate([
        {
          $vectorSearch: {
            index: "vector_index",
...
const generateImageDescription = async (images_base64_strs, image_base64) => {
  ...
  try {
    ...
    const response = await bedrock.invokeModel(params).promise();
    return (
      JSON.parse(response.body).content[0].text || "No description available"
    );
  } catch (error) {
